...
test('終了済み配信を登録し、詳細・一覧・再読込後も確認できる', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '登録済み配信' })).toBeVisible();
  await expect(page.getByRole('heading', { name: '登録済み配信はありません' })).toBeVisible();
...
test('存在しない配信IDでは404状態を表示する', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '配信が見つかりません' })).toBeVisible();
  await expect(page.getByRole('button', { name: '登録済み配信' })).toBeVisible();
});
