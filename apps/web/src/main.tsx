...
function Navigation() {
  ...
  const item = (href: string, label: string, active: boolean) => (
    <a className="nav-link" href={href} aria-current={active ? 'page' : undefined} onClick={(event) => { event.preventDefault(); navigate(href); }}>{label}</a>
  );
  return (
    <nav aria-label="主要ナビゲーション">
      ...
      <div className="nav-links">
        ...
        <a className="nav-create" href="/register" aria-current={path === '/' || path === '/register' ? 'page' : undefined} onClick={(event) => { event.preventDefault(); navigate('/register'); }}>配信を登録</a>
      </div>
    </nav>
  );
}
...
function ListPage() {
  ...
  const visibleStreams = data?.items.filter((stream) => !normalizedQuery || `${stream.title} ${stream.channelTitle}`.toLocaleLowerCase('ja-JP').includes(normalizedQuery)) ?? [];
  return <main><Navigation /><header className="library-header"><div><p className="eyebrow">Stream library</p><h1>配信ライブラリ</h1><p>{data ? `${data.total.toLocaleString('ja-JP')}件の配信` : '収集した配信を探して、場面を確認できます。'}</p></div><button onClick={() => navigate('/register')}>配信を登録</button></header>{loading && <p role="status">読み込み中…</p>}{error && <div><p role="alert" className="error">{error}</p><button onClick={load}>再読み込み</button></div>}{!loading && !error && data?.total === 0 && <section className="empty"><h2>登録済み配信はありません</h2><p>終了済みの配信URLを登録すると、ここから確認できます。</p><button onClick={() => navigate('/register')}>最初の配信を登録</button></section>}{data && data.items.length > 0 && <><div className="library-tools"><label htmlFor="stream-search">配信を検索</label><input id="stream-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="タイトルまたはチャンネル名"/><output aria-live="polite">{visibleStreams.length.toLocaleString('ja-JP')}件を表示</output></div>{visibleStreams.length === 0 ? <section className="empty search-empty-state"><h2>一致する配信はありません</h2><p>検索語を短くするか、チャンネル名で検索してください。</p><button className="secondary" onClick={() => setQuery('')}>検索をクリア</button></section> : <ul className="stream-list" aria-label="登録済み配信">{visibleStreams.map((stream) => <li key={stream.id}><a className="stream-item" href={`/streams/${stream.id}`} onClick={(event) => { event.preventDefault(); navigate(`/streams/${stream.id}`); }}><img src={stream.thumbnailUrl} alt=""/><span className="stream-item-body"><strong>{stream.title}</strong><span className="stream-item-meta"><span>{stream.channelTitle}</span><span><time dateTime={stream.actualStartAt}>{new Date(stream.actualStartAt).toLocaleDateString('ja-JP')}</time> · {duration(stream.durationSeconds)}</span></span><span className="stream-item-action" aria-hidden="true">分析画面を開く <span>→</span></span></span></a></li>)}</ul>}</>}</main>;
}
...
function DetailPage({ id }: { id: string }) {
  ...
  return <main><Navigation />{loading && <p role="status">読み込み中…</p>}{error && <section><h1>配信を表示できません</h1><p role="alert" className="error">{error}</p><button onClick={() => navigate('/streams')}>一覧へ戻る</button></section>}{stream && <><header className="detail-header"><div><p className="eyebrow">Analysis workspace</p><h1>{stream.title}</h1><p>{stream.channelTitle} · {duration(stream.durationSeconds)}</p></div><button className="secondary" onClick={() => navigate('/streams')}>ライブラリへ戻る</button></header><CollectionPanel streamId={stream.id} /><YouTubePlayer videoId={stream.youtubeVideoId} sourceUrl={stream.sourceUrl} durationSeconds={stream.durationSeconds} /><details className="stream-info"><summary>配信情報</summary><StreamCard stream={stream}><p><a href={stream.sourceUrl} target="_blank" rel="noreferrer">YouTubeで元配信を開く</a></p></StreamCard></details></>}</main>;
}
...
function CollectionPanel({ streamId }: { streamId: string }) {
  ...
  return <section className={`collection-strip ${job ? `collection-${job.status}` : ''}`} aria-labelledby="chat-collection-title"><div className="collection-summary"><div><p className="eyebrow">Data collection</p><h2 id="chat-collection-title">{loading ? '収集状態を確認中' : job ? collectionLabels[job.status] : '分析データは未収集です'}</h2></div>{job && <div className="collection-metrics"><span><small>現在工程</small><strong>{currentStep?.name === 'chat_replay' ? 'チャット取得' : currentStep?.name ?? '—'}</strong></span><span><small>取得件数</small><strong>{job.progressCount.toLocaleString('ja-JP')}件</strong></span><span><small>最終更新</small><strong>{timeAgo(job.updatedAt)}</strong></span></div>}</div>{error && <p role="alert" className="error">{error}</p>}{!loading && !job && <div className="collection-callout"><p>チャットと字幕を収集すると、動画と同期して場面を探せます。画面を閉じても処理は継続します。</p><button onClick={start} disabled={submitting}>{submitting ? '開始中…' : '分析データを収集'}</button></div>}{job?.status === 'queued' && <p role="status">ワーカーの開始待ちです。このページを閉じても収集は継続します。</p>}{job?.status === 'running' && !heartbeatStopped && <p role="status">バックグラウンドで収集中です。</p>}{heartbeatStopped && <p role="alert" className="error">ワーカーからの応答が途絶えています。収集が停止している可能性があります。</p>}{job?.status === 'failed' && <div className="collection-callout"><p className="error">{job.errorMessage || currentStep?.errorMessage || 'データ収集に失敗しました。'}</p><button onClick={retry} disabled={submitting}>{submitting ? '再実行中…' : 'データ収集を再実行'}</button></div>}{job?.status === 'succeeded' && <button className="secondary collection-chat-link" onClick={() => navigate(`/streams/${streamId}/chat`)}>チャットだけを一覧表示</button>}</section>;
}
