...
func main() {
	...
	reservationHandler := reservationapi.NewHandler(youtubeClient, reservationapi.NewRepository(db))

	mux := http.NewServeMux()
	mux.HandleFunc("GET /api/health", writeOK)
	mux.HandleFunc("GET /api/ready", func(w http.ResponseWriter, r *http.Request) {
		pingCtx, cancel := context.WithTimeout(r.Context(), 2*time.Second); defer cancel()
		if err := db.Ping(pingCtx); err != nil { http.Error(w, "database unavailable", http.StatusServiceUnavailable); return }
		writeOK(w, r)
	})
	...
	mux.HandleFunc("POST /api/collection-jobs/{jobId}/steps/{stepName}/retry", collectionHandler.RetryStep)
	...
	go func() { log.Printf("Main API listening on %s", addr); if err := server.ListenAndServe(); err != nil && err != http.ErrServerClosed { log.Fatal(err) } }()
	<-ctx.Done()
	shutdownCtx, cancel := context.WithTimeout(context.Background(), 10*time.Second); defer cancel()
	if err := server.Shutdown(shutdownCtx); err != nil { log.Printf("shutdown failed: %v", err) }
}

func writeOK(w http.ResponseWriter, _ *http.Request) { w.Header().Set("Content-Type", "application/json"); _ = json.NewEncoder(w).Encode(healthResponse{Status: "ok"}) }
