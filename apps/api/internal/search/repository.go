...
func (r *Repository) Search(ctx context.Context, streamID, query string, limit int, encodedCursor string) (Page, error) {
	var exists bool
	if err := r.db.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM stream.streams WHERE id=$1)`, streamID).Scan(&exists); err != nil {
		return Page{}, err
	}
	if !exists { return Page{}, ErrStreamNotFound }

	var after cursor
	if encodedCursor != "" {
		decoded, err := base64.RawURLEncoding.DecodeString(encodedCursor)
		if err != nil || json.Unmarshal(decoded, &after) != nil || after.ID == "" || (after.Type != "chat" && after.Type != "transcript") {
	...
	for rows.Next() {
		var item Item
		if err := rows.Scan(&item.ID, &item.Type, &item.ElapsedMilliseconds, &item.Text, &item.AuthorName, &item.LanguageCode, &item.PublishedAt); err != nil {
			return Page{}, err
		}
		items = append(items, item)
	}
	if err := rows.Err(); err != nil { return Page{}, err }
	page := Page{Items: items}
	if len(items) > limit {
		last := items[limit-1]
		page.Items = items[:limit]
		value, _ := json.Marshal(cursor{Elapsed: last.ElapsedMilliseconds, Type: last.Type, ID: last.ID})
